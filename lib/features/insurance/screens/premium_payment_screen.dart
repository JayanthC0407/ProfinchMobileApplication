...
class _PremiumPaymentScreenState extends State<PremiumPaymentScreen> {
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            _infoRow(
              'Next Premium Due',
              dateFmt.format(widget.policy.nextPremiumDue),
            ),
            const SizedBox(height: 8),
            _infoRow(
              'Amount Due',
              '₹${moneyFmt.format(widget.policy.totalPremium)}',
            ),
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text(
                          '₹${moneyFmt.format(account.availableBalance)}',
                          style: TextStyle(
                            fontSize: 12,
                            color: Colors.grey.shade500,
                          ),
            ...
            Row(
              children: [
                ...[
                  widget.policy.totalPremium,
                  widget.policy.totalPremium * 2,
                  widget.policy.totalPremium * 4,
                ].map(
                  (amt) => Padding(
  ...
  void _pay(BuildContext context, double amount) {
    ...
    TransactionProvider.instance.addTransaction(
      ...
      balanceAfter: acctProv
          .getAccountById(widget.policy.debitAccountId)
          .availableBalance,
    );
    context.read<NotificationProvider>().addNotification(
      NotificationModel(
        ...
        body:
            '₹${amount.toStringAsFixed(2)} paid for ${widget.policy.planName}.',
    ...
    AppNotification.show(
      context,
      message:
          'Premium of ₹${NumberFormat('#,##,##0.00', 'en_IN').format(amount)} paid successfully!',
